...
//		File file = new File(split[2]); 
	...
	public void completeSend(String[] split){	//전송이 완료되었음을 알림
		ta.append(split[1]+"에게 파일("+split[4].substring(split[4].lastIndexOf(File.separator)+1)+") 전송을 완료했습니다.\n");
		ta.setCaretPosition(ta.getDocument().getLength());
	...
	public void completeReceive(String[] split){	//전송이 완료되었음을 알림
		ta.append(split[3]+"에게서 파일("+split[2].substring(split[2].lastIndexOf(File.separator)+1)+") 전송을 완료했습니다.\n");
		ta.setCaretPosition(ta.getDocument().getLength());
